# The CalorieTracker server (Telegram bot + upload API)

The original CalorieTracker: a Python service that accepts food photos
from Telegram or phone clients, analyzes them, stores meals in SQLite,
and reports back through Telegram. It predates the Flutter app and now
plays two roles:

1. **The subscription analysis backend for the app** — the app's
   "My server" provider posts photos to this service's `/api/*`
   endpoints, and analysis runs under a Claude, GLM Coding Plan, or
   Doubao Agent Plan subscription instead of a metered API key.
2. **A complete chat-based alternative interface** — everything the app
   does, driven from a Telegram chat instead of a UI. Retire it or run
   it; the knob is `TELEGRAM_POLLING` (default on; `0` keeps the phone
   API alive with no Telegram at all).

Everything below documents running this service.
...
Everything the Flutter app's "your own server" provider uses. All routes
require the `X-API-Key` header (the server's `ANDROID_API_KEY`); analysis
routes are *pure* — they return the analysis and write no meal row, the
phone owns its own database.

| Endpoint | Purpose |
| --- | --- |
| `POST /api/auth_check` | Free readiness probe: validates the key and reports which analysis backends are available. Never spends a CLI run. |
| `POST /api/analyze_photo` | Analyze one photo. The server composes the prompt from its own `shared/` copy — the phone may contribute only a bounded dietary-profile appendix, never the prompt itself. |
| `POST /api/text_intent` | Natural-language intent parsing for corrections and text-described meals. |
| `POST /api/claude_auth/start` / `complete` / `cancel` | Phone-initiated OAuth re-connect for the server's Claude subscription. The PKCE verifier and token never leave the server. |

Response contract worth knowing:

- Every analysis reply carries `analyzed_by` — a receipt naming the
  backend that actually paid. The app refuses a mismatch, so a
  misconfigured server can never silently spend the wrong plan.
- A `503` means one of two very different things: `retry: true` — the
  single-flight CLI was busy, retry in place in a few seconds;
  `retry: false` — the run happened and produced nothing usable, more
  retries would only spend the subscription for the same junk.

## Subscription backends (Claude / GLM / Doubao)

The analyzer runs headless Claude Code CLI; which *plan* pays is per
request, chosen by the app's Claude / GLM / Doubao selector:

| Backend | Server `.env` | Notes |
| --- | --- | --- |
| Claude | `CLAUDE_ANALYZER_ENABLED=1` + `CLAUDE_CODE_OAUTH_TOKEN` (minted via `claude setup-token`) | The original path. Token renewable from the phone via the OAuth re-connect flow. |
| GLM Coding Plan | `GLM_PLAN_KEY` | Rides `ANTHROPIC_BASE_URL` → open.bigmodel.cn. The plan endpoint is text-only, so photos go through Zhipu's official vision MCP (`@z_ai/mcp-server`). |
| Doubao Agent Plan | `DOUBAO_PLAN_KEY` | Rides the Volcengine Ark plan endpoint. |

Both vendors officially support Claude Code on these plans. Vendor runs
scrub `CLAUDE_CODE_OAUTH_TOKEN`/`ANTHROPIC_API_KEY` from the child
environment so a GLM or Doubao run can never accidentally spend the
Anthropic subscription. Check your plan's acceptable-use terms before
routing app traffic through it.

`/api/auth_check` reports per-backend availability, and the app's
diagnostics page ("Test AI provider") walks the whole chain when
something is off.
