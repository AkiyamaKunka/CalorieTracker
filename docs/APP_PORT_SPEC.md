...
| Key checking (rewritten 2026-07-31) | n/a (server key assumed valid) | keys persist ON TYPE (no validate-to-save step; the Validate button is deleted); the one test action is the diagnostics page, whose auth stage uses `AnalyzerService.probeKey` — a four-way classification (ok / outOfCredit / rateLimited / rejected) that validateKey cannot express because quota-class responses count as ACCEPTED there (a 429 proves auth; refusing to save locked onboarding for hours). ServerAnalyzer's probeKey uses the free /api/auth_check, never a CLI run | an empty account must be distinguishable from a healthy one, and a paste-then-tab-away must configure the app | `settings_store_adapter_test`, `server_analyzer_test` probeKey, `diagnostics_test` |
| Server 503 protocol (2026-07-31) | n/a | /api/analyze_photo + /api/text_intent 503s carry `retry`: true = the CLI was BUSY (in-place retry helps), false = the run already happened and produced nothing usable (the app stops at ONE attempt instead of spending two more full CLI runs); the busy signal is an opt-in `AnalyzerBusy` exception so the Telegram path's None-means-fallback contract is untouched; old servers without the field keep conservative no-retry | each wasted retry is up to 120 s of the owner's subscription | `test_api_analyze_endpoints`, `server_analyzer_test` 503 policy |
...
| Data IMPORT (2026-07-31) | export only (`/export` sends a file; no import path) | `MealsDao.importJson` MERGES an exported envelope: per-table dedup keys (PHOTO meal = chat+image_hash so a corrected meal never re-imports as a duplicate of its own backup; photoless meal = chat+date+time+analysis; ledger = chat+hash; weight = chat+date; activity = chat+date+source+external_id+logged_at+the numbers, since many activities share a day), ids dropped so no primary key is reused, `photo_ingestions.meal_id` nulled, unknown tables/rows skipped, whole thing in one transaction; strict format check refuses anything that is not a CalorieTracker export | a re-signed APK or a new phone otherwise means starting the food log from zero — and re-importing the same file must never double the owner's calories | `test/data/import_conformance_test.dart` (real SQLite, two DBs) |
| History gap days (2026-07-31) | server `/history` lists only days WITH meals | app renders interior gap days (oldest logged day → today) as dimmed "no meals logged" rows, tappable to the day editor; leading empty span collapsed | a watcher outage looked identical to a normal list — the missed day simply vanished | `test/ui/history_gaps_test.dart` |
| Provider diagnostics (2026-07-31) | `/doctor` prints server-side state | app-only "Test AI provider" page: six staged checks (config → reachability+VPN hint → auth vs out-of-credit → text round-trip → photo round-trip → quota) streamed live with per-stage evidence and a user-facing fix | a refused backend and a dead network produced the same generic error; diagnosing cost an hour of server-log reading | `test/ui/diagnostics_test.dart` |
| Meals sheet owns corrections (2026-07-31) | Telegram chat is always present for corrections | the pinned Today chat bar was REMOVED; the Meals button's sheet holds all four actions (photo / describe / manual / fix-or-delete → `FixMealScreen`), same §4 executor semantics incl. the delete-confirmation modal | two competing entry points for "change a meal" on one screen | `test/widget_test.dart`, `test/ui/delete_confirmation_test.dart` |
| Model picker (2026-07-31) | server model is an env var | app shows a curated per-provider model dropdown with the tradeoff in the label, plus a Custom row that reveals free text (and is auto-selected for a stored unlisted model) | typing `doubao-seed-2-0-mini-260428` by hand is developer UI | `test/ui/settings_validation_test.dart` |
...
| Body page (2026-08-02) | weight readable only through chat (`/weight`, reports) | a Body tab shows the `body_weight` history (which the executor had been writing with NO UI to read it), a date-proportional trend chart, and add/edit/delete; UI writes carry `source='manual'` (server weigh-ins only ever arrive via chat, `source='nl'`) | the data existed and was invisible — a tracker that hides its own record teaches users to stop weighing | `body_screen_test`, `body_metrics_test` |
...
| Claude-plan model & effort (2026-08-05) | server model fixed by CLAUDE_ANALYZER_MODEL env (owner-set) | Subscription▸Claude plan gains Model (server default/opus/sonnet/haiku) + Thinking effort (server default/low/medium/high) rows; stored as `settings.server_model`/`server_effort` ('' = default), sent with analyze_photo/text_intent/analyze_leftover ONLY on the claude backend; the server validates against CLOSED whitelists (claude_analyzer.CLAUDE_PLAN_MODELS/_EFFORTS — the values become CLI argv, so anything else 400s BEFORE a CLI run, and the argv builder refuses off-list values as defense in depth); vendor plans (glm/doubao) map model names server-side, so overrides are rejected for them and the UI hides the rows | the user wants per-request speed/accuracy control over their own subscription without ssh-ing to change an env var | `plan_choice_test` (bodies claude-only), `settings_validation_test` planTuningTests (UI), `test_plan_model_effort.py` (endpoint 400s, argv whitelists) |
| Leftover deduction (2026-08-05) | none — the server has no before/after concept | Add-sheet "Log leftovers" flow: pick a meal (today/yesterday) + the leftover photo; the model returns per-item left_fractions vs the ORIGINAL analysis (shared/prompts/leftover_prompt.txt, both bindings; server: /api/analyze_leftover composes the prompt from a RE-SANITIZED whitelist — utils.compact_leftover_original — and analyze_leftover_photo never takes the Read fallback and skips the is_food gate); core/leftover_logic.dart does ALL math (clamped 0..1, calories can only shrink, macros scale by the eaten fraction, `leftover.original` snapshot makes re-application base on the original — never compounds); the meal row updates in place so Today/History/reports just work; WATCHER COLLISION: if the leftover photo's hash already became a meal, confirming deletes that duplicate (deleteMeal tombstones the ledger), else the hash is marked skipped so it never will | the real incident 2026-08-04: a leftover photo logged as a NEW ~690 kcal meal ten minutes after the original — the model even wrote "(mostly eaten)" while double-counting | `leftover_logic_test` (math, hostile fractions, re-application), `leftover_flow_test` (incl. the collision repair), `test_api_leftover_endpoint.py` (auth/purity/sanitize/busy) |
